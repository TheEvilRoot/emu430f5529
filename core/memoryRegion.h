...
struct MemoryRegion {
    ...
    MemoryRegion(std::string name, std::uint16_t start, std::uint16_t end, MemoryRegionAccess mode):
     name{std::move(name)}, start_addr{start}, end_addr{end}, access_mode{mode} { }

    MemoryRegion(std::string name, MemoryRegionAccess mode, std::uint16_t start, std::uint16_t length):
     name{std::move(name)}, start_addr{start}, end_addr{std::uint16_t(start + length)}, access_mode{mode} { }

    [[nodiscard]] bool can_access(std::uint16_t virt_addr, core::MemoryRefType ref_type) const {
